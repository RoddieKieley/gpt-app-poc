...
- [ ] T001 Add consent-gate configuration defaults and test-safe overrides in `server.ts`
...
- [ ] T002 [P] Add reusable consent test fixtures/helpers in `tests/integration/sosreport-generate.failures.test.ts` and `tests/integration/sosreport-generate.success.test.ts`
...
- [ ] T003 [P] Add baseline no-secret-log assertions for consent token flows in `tests/regression/no-pat-leakage-mcp.test.ts`
...
- [ ] T004 Create consent token primitives (mint/verify/consume interfaces and claim types) in `src/security/consent-token-service.ts`
...
- [ ] T005 [P] Create centralized sensitive-tool policy decision layer in `src/security/sensitive-tool-policy.ts`
...
- [ ] T006 [P] Extend security event/redaction surfaces for consent-denial logging in `src/security/security-events.ts` and `src/security/redaction.ts`
...
- [ ] T007 Add foundational unit coverage for token service and policy module in `tests/unit/consent-token-service.test.ts` and `tests/unit/sensitive-tool-policy.test.ts`
...
- [ ] T008 [P] [US1] Add contract assertions for required `consent_token` input on `generate_sosreport` in `tests/contract/sosreport-tools.contract.test.ts`
...
- [ ] T009 [P] [US1] Add integration denial matrix (missing, invalid, expired, replay, wrong-user, wrong-scope) in `tests/integration/sosreport-generate.failures.test.ts`
...
- [ ] T010 [P] [US1] Add integration happy path for single valid token use in `tests/integration/sosreport-generate.success.test.ts`
...
- [ ] T011 [US1] Implement secure consent mint endpoint for explicit Step 2 action in `server.ts`
...
- [ ] T012 [US1] Implement signed claim verification and one-time consume semantics in `src/security/consent-token-service.ts`
...
- [ ] T013 [US1] Require `consent_token` in `generate_sosreport` schema in `src/sosreport/sosreport-tool-schemas.ts`
...
- [ ] T014 [US1] Wire consent authorization before diagnostics execution in `server.ts` and `src/sosreport/sosreport-tool-handlers.ts`
...
- [ ] T015 [P] [US2] Add policy-focused contract checks in `tests/contract/engage-red-hat-support.contract.test.ts`
...
- [ ] T016 [P] [US2] Add integration test that policy denial blocks diagnostics start in `tests/integration/sosreport-generate.failures.test.ts`
...
- [ ] T017 [US2] Integrate centralized policy middleware into `generate_sosreport` MCP registration in `server.ts`
...
- [ ] T018 [US2] Implement standardized safe deny response mapping in `src/security/sensitive-tool-policy.ts` and `src/security/redaction.ts`
...
- [ ] T019 [US2] Emit policy decision telemetry for allow/deny/replay outcomes in `src/security/security-events.ts` and `server.ts`
...
- [ ] T020 [P] [US3] Add UI workflow integration assertions for explicit Step 2 click behavior in `tests/integration/engage-red-hat-support.workflow.test.ts`
...
- [ ] T021 [P] [US3] Add regression checks for unchanged unrelated tools/resources and entrypoint stability in `tests/regression/mcp-tool-surface-preservation.test.ts` and `tests/regression/skill-resource-preservation.test.ts`
...
- [ ] T022 [US3] Update Step 2 Generate UI handler to mint consent then call `generate_sosreport(consent_token)` in `src/mcp-app.ts`
...
- [ ] T023 [US3] Enforce no automatic diagnostics collection during load/navigation in `src/mcp-app.ts`
...
- [ ] T024 [US3] Update non-UI fallback and operator guidance in `skills/engage-red-hat-support/SKILL.md`, `README.md`, and `docs/operator-guide.md`
...
- [ ] T026 [P] Add/refresh consent policy contract artifacts in `specs/011-consent-gate-sosreport/contracts/consent-endpoint.openapi.yaml` and `specs/011-consent-gate-sosreport/contracts/sensitive-tool-policy.contract.json`
...
- [ ] T027 Run full verification suite via `npm run test:unit`, `npm run test:contract`, `npm run test:integration`, and `npm run test:regression` from repo root
...
- [ ] T028 Record verification outcomes and residual risk notes in `specs/011-consent-gate-sosreport/quickstart.md` and `specs/011-consent-gate-sosreport/plan.md`
