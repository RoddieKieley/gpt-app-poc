...
- [x] T001 [US1] Add mint tool input schema/type for optional validated workflow session in `src/sosreport/sosreport-tool-schemas.ts`
...
- [x] T002 [US1] Register `mint_engage_consent_token` MCP tool with standard response shape in `server.ts`
...
- [x] T003 [US3] Enforce optional `workflow_session_id` strict ownership/session validation in mint path in `server.ts`
...
- [x] T004 [US1] Enforce mint-after-step1 precondition in MCP path using existing workflow state helpers in `server.ts`
...
- [x] T005 [US1] Preserve existing generate consent policy and add optional session-alignment check in `server.ts` and `src/security/sensitive-tool-policy.ts`
...
- [x] T006 [US2] Verify web endpoint behavior remains unchanged for `POST /api/engage/consent-tokens` in `server.ts`
...
- [x] T007 [P] [US1] Update MCP/tool contract assertions for new mint tool in `tests/contract/sosreport-tools.contract.test.ts`
...
- [x] T008 [P] [US2] Update MCP surface regression baseline to include additive tool only in `tests/regression/mcp-tool-surface-preservation.test.ts`
...
- [x] T009 [P] [US1] Add new versioned tool contract file `specs/013-mcp-consent-mint-path/contracts/engage-consent-mint-mcp.contract.v1.json`
...
- [x] T010 [P] [US1] Add new versioned headless workflow sequence contract in `specs/013-mcp-consent-mint-path/contracts/engage-workflow-headless-sequence.contract.v1.json`
...
- [x] T011 [P] [US2] Add parity contract for unchanged web endpoint + mint response shape in `specs/013-mcp-consent-mint-path/contracts/consent-mint-parity.openapi.v1.yaml`
...
- [x] T012 [P] [US1] Update headless/text guidance with explicit mint invocation in `specs/013-mcp-consent-mint-path/quickstart.md`
...
- [x] T013 [P] [US2] Update regression expectations for unchanged web behavior in `specs/013-mcp-consent-mint-path/plan.md` and `specs/013-mcp-consent-mint-path/spec.md` (if clarifying notes needed)
...
- [x] T014 [P] [US1] Add helper for MCP mint tool invocation in `tests/integration/consent-test-helpers.ts`
...
- [x] T015 [US1] Add headless happy-path integration sequence in `tests/integration/sosreport-generate.success.test.ts`
...
- [x] T016 [US1] Add denial tests for mint-before-step1 and missing/invalid consent in `tests/integration/sosreport-generate.failures.test.ts`
...
- [x] T017 [US3] Add invalid `workflow_session_id` denial and mismatch scenarios in `tests/integration/sosreport-generate.failures.test.ts`
...
- [x] T018 [US1] Extend existing denial matrix to reassert replay/wrong-user/wrong-session/wrong-scope/wrong-step in `tests/integration/sosreport-generate.failures.test.ts`
...
- [x] T019 [US2] Add web regression assertion coverage for unchanged consent endpoint behavior in `tests/integration/engage-red-hat-support.workflow.test.ts`
...
- [x] T020 Run targeted verification suite for mint flow and denial matrix using `tsx --test` on integration/contract/regression files
...
- [x] T021 Run full regression verification and record results via `npm run test:unit && npm run test:contract && npm run test:integration && npm run test:regression`
...
- [x] T022 Release-readiness check and rollout sign-off in `specs/013-mcp-consent-mint-path/quickstart.md` and release notes/PR description
