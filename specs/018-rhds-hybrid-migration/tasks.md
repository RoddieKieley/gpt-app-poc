...
- [ ] B1 checkpoint passed (behavior parity + accessibility sanity + visual consistency + rollback drill)
- [ ] B2 checkpoint passed (behavior parity + accessibility sanity + visual consistency + rollback drill)
- [ ] B3 checkpoint passed or explicitly deferred with rationale and mapping update
- [ ] `src/mcp-app.ts` contracts unchanged
- [ ] Callback signatures in `src/mcp-app/App.tsx` and `src/mcp-app/step-content.tsx` unchanged
- [ ] Existing build/serve behavior confirmed unchanged
- [ ] Step 1 mapping docs updated in `specs/018-rhds-hybrid-migration/contracts/engage-ui-component-map.step1.v1.json`
- [ ] Historical contract `specs/012-patternfly-ui-swap/contracts/engage-ui-component-map.v1.json` remains unmodified

## Explicit Prerequisites to Begin Step 2

- [ ] All Step 1 go/no-go checkpoints are resolved (passed or explicitly deferred with accepted rationale)
- [ ] No unresolved critical regressions remain in behavior, accessibility, or visual checks
- [ ] Rollback procedures for all substituted Step 1 surfaces are documented and validated
- [ ] Hybrid ownership map is current and includes Step 2 candidate notes
